 ...
 * 还有一个word1和word2此时的元素都删除，即dp[i-1][j-1] + 2，
 * 1. 这个可以被dp[i][j-1] + 1覆盖掉，表示删除了word2的j-1位置的元素，
 * 2. 如果此时要删除word1的i - 1位置的元素，那么表示为dp[i-1][j-1] + 1 + 1 => 推出dp[i-1][j-1] + 2.
 * 同理，也可以由dp[i -1][j] + 1推出。
 * 3、初始化
...
public class L37_583_minDistance {
     ...
     * @param word2:
     * @return int
     * @description 1. dp公式和之前不一样， 相等时,不动;不等时,两个字符串挑一个得删一次;
