 ...
 * 1. dp[i]表示第i个屋子小偷能偷的最大钱；
 * 2. dp[i] = Math.max(dp[i - 1], dp[i - 2] + nums[i])
 * 遍历范围从start, start+1, end;
 * 3. 初始化: dp[start] = nums[start] dp[start + 1] = Math.max(nums[start], nums[start + 1]);
...
public class L21_213_rob2 {
    ...
    public int rob2(int[] nums) {
        ...
        int max1 = robRange2(nums, 0, nums.length - 2);
        int max2 = robRange2(nums, 1, nums.length - 1);

        return Math.max(max1, max2);
    }
    private int robRange2(int[] nums, int left, int right) {
        if (left == right) return nums[left];
        int[] dp = new int[nums.length]; // 只要长度大于right - left + 2就行;
        dp[left] = nums[left];
