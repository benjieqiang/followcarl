...
public class L25_123_maxProfit {
    // dp
    /*
      1. dp数组的含义 定义5种状态;dp[i][j]中 i表示第i天，j为 [0 - 4] 五个状态，dp[i][j]表示第i天状态j所剩最大现金。
      // dp[i][0] 表示第i天持有的最大现金，不操作，也可以不设置该状态，因为没有操作，手上的现金自然就是0
      dp[i][1] 表示第一次持有：在前i-1天就持有了 vs 到第i天才买入股票。
      dp[i][2] 表示第一次不持有：在前i-1天就卖了，到了第i天不操作 vs 前i-1天持有，等到第i天才进行卖出股票；
      dp[i][3] 表示第二次持有；在前i-1天就完成了买入和卖出操作，一直维持到第i天 vs 到了第i天先卖出第一次持有的股票进行第二次买入
      dp[i][4] 表示第二次不持有：在前i-1天完成了两次买入卖出的操作 vs 到第i天先买入第二次股票，再卖出第二次
      2.
      dp[i][0] = dp[i - 1][0]
    ...
    public int maxProfit(int[] prices) {
        ...
        for (int i = 1; i < prices.length; i++) {
            dp[i][1] = Math.max(dp[i-1][1], -prices[i]); // 要么保持前i-1天状态, 即买入的状态；要么在第i天再买入;
