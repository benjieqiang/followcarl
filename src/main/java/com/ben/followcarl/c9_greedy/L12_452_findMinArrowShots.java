 ...
 * 1. 先按照左边界从小到大排序；
 * 2. 比较当前左边界和前右边界，大于则res加1，res从1开始，表示至少需要射一次；
 * 3. 小于等于则更新最小右边界；
 *
 * @Version: 1.0
 */
public class L12_452_findMinArrowShots {
    public int findMinArrowShots(int[][] points) {
        ...
        for (int i = 1; i < points.length; i++) {
            // 不重合：当前左边界比前右边界值大，说明需要再用一个弓箭射中当前的气球；不加等号原因是：两个气球挨在一起xstart ≤ x ≤ xend也可以一起射爆，
            if (points[i][0] > points[i - 1][1]) {
                res++;
            } else {
                // <= 当前气球的左边界和前右边界有重叠，需要更新当前气球的右边界，
                // 因为这个右边界在下一次for循环中可以用来判断是否和下一个气球左边界重合，如果有重合，只需要一个弓箭就可以射
