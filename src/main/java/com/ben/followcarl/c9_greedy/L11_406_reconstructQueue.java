 ...
 * @Description:
 * h是身高，k是前面有k个人身高大于h；{7,0},{4,4},{7,1},{5,0},{6,1},{5,2}
...
public class L11_406_reconstructQueue {
    public int[][] reconstructQueue(int[][] people) {
        // 身高从大到小排（如果身高h相同, 那么k按照从小到大排，k小的站前面）注意：sort里面的自定义排序，返回值=0，a=b; <0, a小于b；>0, a大于b。
...
//        }
     ...
     *
     * 时间复杂度 O(n^2) 其中 n 是数组 people的长度。我们需要 O(nlogn) 的时间进行排序，随后需要 O(n2)的时间遍历每一个人并将他们放入队列中。
     * 由于前者在渐近意义下小于后者，因此总时间复杂度为 O(n2)
