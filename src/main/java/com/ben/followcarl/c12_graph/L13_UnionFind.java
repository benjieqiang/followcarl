 ...
 *
 * 1. 并查集初始化：让集合n所有节点自己指向自己；
 * 2. 路径压缩的find函数：如果u就是father[u], 则直接返回，不用找了，否则，让父节点father[u]一层层接收find(father[u])的父节点
 * 3. join函数，分别寻找两者的根节点，让前者成为后者的父节点；
 * 4. isSame，分别找到u和v的根，判断是否相等；
 * @Version: 1.0
 */
